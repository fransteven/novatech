...
- **Desglose mensual auditable**: `getMonthlyProfitBreakdown` ya no filtra `interest_portion > 0` — escondía los abonos a capital y hacía imposible cuadrar la sección contra la caja (listaba 7 de 16 pagos del mes).
- **Límites de período exclusivos**: `endOfMonth`/`endOfYear` dan `23:59:59.999` y los timestamps tienen precisión de microsegundos, así que `lte` podía perder filas. Ganancias y `getSalesKPIs` usan `>= from AND < toExclusive`.
- **Fecha de compra futura**: `createPurchaseSchema` rechaza `purchaseDate` posterior al día en curso.
- **Ganancias en móvil**: los KPIs se leen como estado de resultados con la utilidad neta de titular (antes 8 tarjetas apiladas = 4 pantallas) y el desglose mensual pasa de una tabla de 9 columnas con scroll horizontal de 780px a tarjetas por mes; el scroll anidado de las secciones del desglose sólo aplica desde `sm`.

> [!WARNING]
> **Pendiente — desfase de zona horaria en el corte de mes.** `getMonthlyProfits` agrupa con `EXTRACT(MONTH FROM created_at)` (timestamp crudo, UTC) mientras `getProfitsKPIs` y `getMonthlyProfitBreakdown` filtran con límites de `America/Bogota`. Las filas entre las 00:00 y las 05:00 UTC del día 1 caen en meses distintos según el camino: en 2026 la fila de septiembre y su desglose difieren en $1.800.000 de ventas y $149.713 de intereses. Unificar el criterio (idealmente migrando las columnas a `timestamptz`).
