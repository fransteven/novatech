 ...
 * LLM_PROVIDER = "gemini"              → GeminiProvider (GEMINI_API_KEY, LLM_MODEL opcional)
...
export function getLLMProvider(): LLMProvider {
  ...
  if (!provider) {
    throw new Error(
      "No hay proveedor LLM configurado. " +
        "Agrega LLM_PROVIDER=claude (con ANTHROPIC_API_KEY), " +
        "LLM_PROVIDER=gemini (con GEMINI_API_KEY) o " +
  ...
  switch (provider) {
    ...
    case "gemini": {
      const { GeminiProvider } = require("./gemini-provider") as {
    ...
    default:
      throw new Error(
        `Proveedor LLM desconocido: "${provider}". Usa "claude", "gemini", "deepseek" u "openai-compatible".`
      );
  }
}
